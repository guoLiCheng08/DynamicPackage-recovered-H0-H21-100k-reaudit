...
#define STEP_COUNT 10u
#else
#define STATE_GOLD "gold_high_ecc_thirdq_inverseflex_hundred_step_state.bin"
#define MAIN_GOLD "gold_high_ecc_thirdq_inverseflex_hundred_step_out.bin"
#define IPC_GOLD "gold_high_ecc_thirdq_inverseflex_hundred_step_ipc_payload.bin"
#define SCENARIO_LABEL "high-eccentric third-quaternion inverse-flex hundred-step"
...
int main(void)
{
    ...
    if (expected_states == NULL || expected_main == NULL || expected_ipc == NULL ||
        ...
        read_gold("gold_high_ecc_thirdq_inverseflex_hundred_step_global_y.bin", expected_global_y,
...
/* 原 ELF 联合探针保持零设备命令。 */
    ...
    for (step = 0u; step < STEP_COUNT; ++step) {
        ...
        {
            FILE *dss_trace = fopen("/tmp/h11_recovered_dss_trace.bin",
                                    step == 0u ? "wb" : "ab");
            if (dss_trace == NULL ||
                fwrite(DeviceMeasure.dss, 1u, sizeof(DeviceMeasure.dss), dss_trace) !=
