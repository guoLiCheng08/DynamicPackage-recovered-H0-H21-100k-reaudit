...
int main(void)
{
    ...
    int actual_result[3];
    double source_data[3] = {1.5, -2.0, 0.25};
    double destination_data[3] = {10.0, 20.0, -30.0};
    DpVector source = {3, 0, source_data};
    DpVector destination = {2, 0, destination_data};
    int mismatch = 0;

    if (read_blob("analysis/time_orbit/gold_vector_axpy_nonfatal_p3_result.bin",
                  expected_result, sizeof(expected_result)) != 0 ||
        read_blob("analysis/time_orbit/gold_vector_axpy_nonfatal_p3_destination.bin",
                  expected_destination, sizeof(expected_destination)) != 0) {
        return 1;
    }

    actual_result[0] = vector_axpy(&source, 0.0, &destination);
    source.count = 0;
    actual_result[1] = vector_axpy(&source, 2.0, &destination);
    source.count = 3;
    destination.count = 2;
    actual_result[2] = vector_axpy(&source, -0.5, &destination);

    mismatch |= compare_blob("vector_axpy P3 returns", actual_result, expected_result,
                             sizeof(actual_result));
    mismatch |= compare_blob("vector_axpy P3 destination", destination_data,
                             expected_destination, sizeof(destination_data));
    if (mismatch == 0) {
        puts("vector_axpy nonfatal P3 original-ELF compare: PASS (bitwise)");
    }
