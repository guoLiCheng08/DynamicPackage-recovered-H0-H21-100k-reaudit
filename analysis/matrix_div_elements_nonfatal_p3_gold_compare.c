...
int main(void)
{
    ...
    double left_data[8] = {3.0,-6.0,90.0,5.0,0.0,91.0,0.0,0.0};
    double right_data[10] = {2.0,-3.0,30.0,5.0,0.0,60.0,70.0,80.0,0.0,0.0};
    DpMatrix left = {2, 2, 3, 0, left_data};
    DpMatrix right = {3, 2, 4, 0, right_data};
    int result;
    int mismatch = 0;

    if (read_exact("gold_matrix_div_elements_nonfatal_p3_results.bin", expected_results,
                   sizeof(expected_results)) != 0 ||
        read_exact("gold_matrix_div_elements_nonfatal_p3_left.bin", expected_left,
                   sizeof(expected_left)) != 0) return 1;
    result = matrix_div_elements(&left, &right);
    memcpy(actual_results, &result, sizeof(result));
    right.rows = 2;
    result = matrix_div_elements(&left, &right);
    memcpy(actual_results + 4u, &result, sizeof(result));
    mismatch |= compare_blob("matrix_div_elements P3 returns", actual_results, expected_results,
                             sizeof(actual_results));
    mismatch |= compare_blob("matrix_div_elements P3 left", (const unsigned char *)left_data,
                             (const unsigned char *)expected_left, sizeof(left_data));
    if (mismatch == 0) puts("matrix_div_elements nonfatal P3 original-ELF compare: PASS (bitwise)");
