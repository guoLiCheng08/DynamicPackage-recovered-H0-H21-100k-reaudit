...
int main(void)
{
    ...
    if (read_gold("gold_dyn_init_array_h28_device_first_rwheel.bin", expected_rwheel,
                  sizeof(expected_rwheel)) != 0 ||
        read_gold("gold_dyn_init_array_h28_device_first_mtq.bin", expected_mtq,
                  sizeof(expected_mtq)) != 0 ||
        read_gold("gold_dyn_init_array_h28_device_first_sada.bin", expected_sada,
                  sizeof(expected_sada)) != 0) {
        return 1;
    }
    dyn_init_array(initial);
    mismatch = compare_blob("H28 dyn_init_array RWheel", RWheel, expected_rwheel,
                            sizeof(expected_rwheel));
    mismatch |= compare_blob("H28 dyn_init_array MTQ", MTQ, expected_mtq,
                             sizeof(expected_mtq));
    mismatch |= compare_blob("H28 dyn_init_array SADA", &SADA, expected_sada,
