...
int main(void)
{
    ...
    if (gold_state == NULL || gold_y == NULL || gold_main == NULL || gold_ipc == NULL ||
        read_gold("gold_startup_chain_h30_hundred_first_state.bin", gold_state, STEP_COUNT * STATE_BYTES) != 0 ||
        read_gold("gold_startup_chain_h30_hundred_first_global_y.bin", gold_y, STEP_COUNT * GLOBAL_Y_BYTES) != 0 ||
        read_gold("gold_startup_chain_h30_hundred_first_out.bin", gold_main, STEP_COUNT * MAIN_BYTES) != 0 ||
        read_gold("gold_startup_chain_h30_hundred_first_ipc_payload.bin", gold_ipc, STEP_COUNT * IPC_BYTES) != 0) {
    ...
    free(gold_state); free(gold_y); free(gold_main); free(gold_ipc);
    if (mismatch == 0) {
        (void)puts("PASS: H30 startup chain hundred-step bitwise compare");
        return 0;
    }
    return 1;
}
