...
int main(void)
{
    ...
    if (expected_states == NULL || expected_main == NULL || expected_ipc == NULL ||
        ...
        read_gold("gold_dss_custom_noise_seed12345_thousand_step_state.bin", expected_states, STEP_COUNT * STATE_BYTES) ||
        read_gold("gold_dss_custom_noise_seed12345_thousand_step_out.bin", expected_main, STEP_COUNT * MAIN_BYTES) ||
        read_gold("gold_dss_custom_noise_seed12345_thousand_step_ipc_payload.bin", expected_ipc, STEP_COUNT * IPC_PAYLOAD_BYTES) ||
        read_gold("gold_dss_custom_noise_seed12345_h25_global_y_thousand_first_global_y.bin", expected_global_y,
    ...
    DeviceMeasure.dss[1].gaussian_noise_flag = 1u;
    DeviceMeasure.dss[0].gaussian_mean[0] = 0.002;
    DeviceMeasure.dss[0].gaussian_mean[1] = -0.003;
    DeviceMeasure.dss[0].gaussian_sigma[0] = 0.020;
    DeviceMeasure.dss[0].gaussian_sigma[1] = 0.030;
    DeviceMeasure.dss[1].gaussian_mean[0] = -0.004;
    DeviceMeasure.dss[1].gaussian_mean[1] = 0.005;
    DeviceMeasure.dss[1].gaussian_sigma[0] = 0.025;
    DeviceMeasure.dss[1].gaussian_sigma[1] = 0.015;
    ...
    for (step = 0u; step < STEP_COUNT; ++step) {
        ...
        (void)snprintf(label, sizeof(label), "DSS-custom-noise-seed12345 thousand-step[%u] CoreDynamic first 0x108", step + 1u);
        mismatch |= compare_blob(label, core_output, expected_states + step * STATE_BYTES, STATE_BYTES);
        (void)snprintf(label, sizeof(label), "DSS-custom-noise-seed12345 thousand-step[%u] main output", step + 1u);
        ...
        (void)snprintf(label, sizeof(label), "DSS-custom-noise-seed12345 thousand-step[%u] IPC payload", step + 1u);
        mismatch |= compare_blob(label, actual_ipc.raw + DP_IPC_FLOAT_BASE,
                                 expected_ipc + step * IPC_PAYLOAD_BYTES, IPC_PAYLOAD_BYTES);
        (void)snprintf(label, sizeof(label), "H25 DSS global-y thousand-step[%u] global y", step + 1u);
    ...
    free(expected_global_y);
    if (mismatch == 0) {
        puts("dyn_main DSS-custom-noise-seed12345 thousand-step original-ELF compare: PASS (bitwise)");
    }
