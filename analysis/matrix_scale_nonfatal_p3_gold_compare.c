...
int main(void)
{
    ...
    if (read_exact("gold_matrix_scale_nonfatal_p3_result.bin", expected_result,
                   sizeof(expected_result)) != 0 ||
        read_exact("gold_matrix_scale_nonfatal_p3_data.bin", expected_data,
                   sizeof(expected_data)) != 0) return 1;
    for (index = 0u; index < 12u; ++index) data[index] = (double)index + 1.0;
    data[1] = 0.0;
    result = matrix_scale(&matrix, -2.0);
    memcpy(actual_result, &result, sizeof(result));
    mismatch |= compare_blob("matrix_scale P3 return", actual_result, expected_result,
                             sizeof(actual_result));
    mismatch |= compare_blob("matrix_scale P3 data", (const unsigned char *)data,
                             (const unsigned char *)expected_data, sizeof(data));
    if (mismatch == 0) puts("matrix_scale nonfatal P3 original-ELF compare: PASS (bitwise)");
