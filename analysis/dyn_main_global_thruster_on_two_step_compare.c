...
int main(void)
{
    ...
    if (read_gold("gold_sensor_init_gyro.bin", DeviceMeasure.gyro, sizeof(DeviceMeasure.gyro)) ||
        ...
        read_gold("gold_thruster_on_step1_state.bin", expected_state, sizeof(expected_state)) ||
        read_gold("gold_thruster_on_step1_out.bin", &expected_out, sizeof(expected_out)) ||
        read_gold("gold_thruster_on_step1_ipc.bin", &expected_ipc, sizeof(expected_ipc)) ||
        read_gold("gold_thruster_on_step1_thruster.bin", &expected_thruster, sizeof(expected_thruster)) ||
        read_gold("gold_thruster_on_step1_thruster_vectors.bin", expected_thruster_vectors,
                  sizeof(expected_thruster_vectors)) ||
        read_gold("gold_thruster_on_step2_state.bin", expected_state_step2, sizeof(expected_state_step2)) ||
        read_gold("gold_thruster_on_step2_out.bin", &expected_out_step2, sizeof(expected_out_step2)) ||
        read_gold("gold_thruster_on_step2_ipc.bin", &expected_ipc_step2, sizeof(expected_ipc_step2)) ||
        read_gold("gold_thruster_on_step2_thruster.bin", &expected_thruster_step2, sizeof(expected_thruster_step2)) ||
        read_gold("gold_thruster_on_step2_thruster_vectors.bin", expected_thruster_vectors_step2,
    ...
    mismatch |= compare_blob("thruster step1 CoreDynamic first 0x108", core_output,
                             expected_state, sizeof(expected_state));
    mismatch |= compare_blob("thruster step1 main output", main_output.raw,
                             expected_out.raw, sizeof(main_output.raw));
    mismatch |= compare_thruster_semantic("thruster step1 force scale", &Thruster,
                                          &expected_thruster, expected_thruster_vectors);
    dp_send_dyn_tele(&actual_ipc, &main_output, 0u, 0u, 0u, 0u, 0u);
    mismatch |= compare_blob("thruster step1 IPC payload", actual_ipc.raw + DP_IPC_FLOAT_BASE,
                             expected_ipc.raw + DP_IPC_FLOAT_BASE,
                             DP_IPC_SHM_BYTES - DP_IPC_FLOAT_BASE);

    dyn_main(&main_output, core_output, &command);
    mismatch |= compare_blob("thruster step2 CoreDynamic first 0x108", core_output,
                             expected_state_step2, sizeof(expected_state_step2));
    mismatch |= compare_blob("thruster step2 main output", main_output.raw,
                             expected_out_step2.raw, sizeof(main_output.raw));
    mismatch |= compare_thruster_semantic("thruster step2 force scale", &Thruster,
                                          &expected_thruster_step2, expected_thruster_vectors_step2);
    memset(&actual_ipc, 0, sizeof(actual_ipc));
    dp_send_dyn_tele(&actual_ipc, &main_output, 0u, 0u, 0u, 0u, 0u);
    mismatch |= compare_blob("thruster step2 IPC payload", actual_ipc.raw + DP_IPC_FLOAT_BASE,
                             expected_ipc_step2.raw + DP_IPC_FLOAT_BASE,
                             DP_IPC_SHM_BYTES - DP_IPC_FLOAT_BASE);
    if (mismatch == 0) {
        puts("dyn_main thruster-on two-step original-ELF compare: PASS (bitwise)");
    }
