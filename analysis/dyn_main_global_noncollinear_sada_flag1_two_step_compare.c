...
#ifdef DP_NONCOL_WHEEL_LIMIT
#define S1_STATE "gold_noncollinear_wheel_limit_step1_state.bin"
#define S1_OUT "gold_noncollinear_wheel_limit_step1_out.bin"
#define S1_IPC "gold_noncollinear_wheel_limit_step1_ipc_payload.bin"
#define S2_STATE "gold_noncollinear_wheel_limit_step2_state.bin"
#define S2_OUT "gold_noncollinear_wheel_limit_step2_out.bin"
#define S2_IPC "gold_noncollinear_wheel_limit_step2_ipc_payload.bin"
#else
#define S1_STATE "gold_noncollinear_sada_flag1_step1_state.bin"
#define S1_OUT "gold_noncollinear_sada_flag1_step1_out.bin"
#define S1_IPC "gold_noncollinear_sada_flag1_step1_ipc.bin"
#define S2_STATE "gold_noncollinear_sada_flag1_step2_state.bin"
#define S2_OUT "gold_noncollinear_sada_flag1_step2_out.bin"
#define S2_IPC "gold_noncollinear_sada_flag1_step2_ipc.bin"
#endif
...
int main(void)
{
    ...
    if (read_blob("gold_sensor_init_gyro.bin", DeviceMeasure.gyro,
        ...
        read_blob(S1_STATE, expected_core_prefix, sizeof(expected_core_prefix)) != 0 ||
        read_blob(S1_OUT, &expected_main, sizeof(expected_main)) != 0 ||
        read_blob(S2_STATE, expected_core_prefix_step2, sizeof(expected_core_prefix_step2)) != 0 ||
        read_blob(S2_OUT, &expected_main_step2, sizeof(expected_main_step2)) != 0 ||
#ifdef DP_NONCOL_WHEEL_LIMIT
        read_blob(S1_IPC, expected_ipc_step1.raw + DP_IPC_FLOAT_BASE,
                  DP_IPC_SHM_BYTES - DP_IPC_FLOAT_BASE) != 0 ||
        read_blob(S2_IPC, expected_ipc_step2.raw + DP_IPC_FLOAT_BASE,
                  DP_IPC_SHM_BYTES - DP_IPC_FLOAT_BASE) != 0) {
#else
        read_blob(S1_IPC, &expected_ipc_step1, sizeof(expected_ipc_step1)) != 0 ||
        read_blob(S2_IPC, &expected_ipc_step2, sizeof(expected_ipc_step2)) != 0) {
#endif
        return 1;
    }
...
#ifdef DP_NONCOL_SADA_DIAGNOSTIC
    {
        FILE *sada_file = fopen("/tmp/noncollinear_sada_flag1_actual_sada.bin", "wb");
        if (sada_file == NULL || fwrite(&SADA, 1u, sizeof(SADA), sada_file) != sizeof(SADA) ||
            fclose(sada_file) != 0) {
            return 1;
        }
    }
    ...
    mismatch |= compare_blob("dyn_main step2 IPC payload", actual_ipc.raw + DP_IPC_FLOAT_BASE,
                             ...
                             DP_IPC_SHM_BYTES - DP_IPC_FLOAT_BASE);
    if (mismatch == 0) {
        puts("dyn_main noncollinear-SADA-flag1 two-step original-ELF compare: PASS (bitwise)");
    }
