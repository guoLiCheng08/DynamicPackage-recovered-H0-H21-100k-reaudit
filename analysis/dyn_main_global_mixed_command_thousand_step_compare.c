...
int main(void)
{
    ...
    if (expected_states == NULL || expected_main == NULL || expected_ipc == NULL ||
        ...
        read_gold("gold_mixed_command_thousand_step_state.bin", expected_states, STEP_COUNT * STATE_BYTES) ||
        read_gold("gold_mixed_command_thousand_step_global_y.bin", expected_global_y, STEP_COUNT * STATE_BYTES) ||
        read_gold("gold_mixed_command_thousand_step_out.bin", expected_main, STEP_COUNT * MAIN_BYTES) ||
        read_gold("gold_mixed_command_thousand_step_ipc_payload.bin", expected_ipc, STEP_COUNT * IPC_PAYLOAD_BYTES)) {
...
#ifdef DP_MIXED_DIAGNOSTIC_DUMP
        if (step == 4u || step == 5u) {
            char path[128];
            FILE *file;
            unsigned actual_step = step + 1u;
#define DP_DUMP_MIXED_OBJECT(suffix, object) \
            do { \
                (void)snprintf(path, sizeof(path), "/tmp/mixed_actual_step%u_" suffix ".bin", actual_step); \
                file = fopen(path, "wb"); \
                if (file == NULL || fwrite(&(object), 1u, sizeof(object), file) != sizeof(object) || \
                    (file != NULL && fclose(file) != 0)) { \
                    free(expected_states); free(expected_main); free(expected_ipc); return 1; \
                } \
            } while (0)
            DP_DUMP_MIXED_OBJECT("sada", SADA);
            DP_DUMP_MIXED_OBJECT("rwheel", RWheel);
            DP_DUMP_MIXED_OBJECT("mtq", MTQ);
            DP_DUMP_MIXED_OBJECT("j", J_c_B_mem);
            DP_DUMP_MIXED_OBJECT("jinv", J_c_B_inv_mem);
            DP_DUMP_MIXED_OBJECT("h", H_w_B_mem);
            DP_DUMP_MIXED_OBJECT("l", L_c_B_mem);
#undef DP_DUMP_MIXED_OBJECT
            (void)snprintf(path, sizeof(path), "/tmp/mixed_actual_step%u_f.bin", actual_step);
            file = fopen(path, "wb");
            if (file == NULL || F_I_external.data == NULL ||
                fwrite(F_I_external.data, 1u, 24u, file) != 24u || fclose(file) != 0) {
                free(expected_states); free(expected_main); free(expected_ipc); return 1;
            }
            if (step == 4u || step == 5u) {
                static const size_t descriptor_offsets[6] = {
                    0x4a8u, 0x688u, 0x898u, 0x9a0u, 0xa00u, 0xd38u
                };
                static const size_t descriptor_bytes[6] = {
                    72u, 240u, 240u, 72u, 800u, 800u
                };
                unsigned matrix_index;
                (void)snprintf(path, sizeof(path), "/tmp/mixed_actual_step%u_model_used.bin", actual_step);
                file = fopen(path, "wb");
                if (file == NULL) {
                    free(expected_states); free(expected_main); free(expected_ipc); return 1;
                }
                for (matrix_index = 0u; matrix_index < 6u; ++matrix_index) {
                    DpMatrix matrix;
                    memcpy(&matrix, &Sat.raw[descriptor_offsets[matrix_index]], sizeof(matrix));
                    if (matrix.data == NULL || fwrite(matrix.data, 1u, descriptor_bytes[matrix_index], file) !=
                                                   descriptor_bytes[matrix_index]) {
                        (void)fclose(file);
                        free(expected_states); free(expected_main); free(expected_ipc); return 1;
                    }
                }
...
#ifdef DP_MIXED_DIAGNOSTIC_DUMP
        if (step == 4u) {
            FILE *state_file = fopen("/tmp/mixed_actual_step5_state.bin", "wb");
            if (state_file == NULL || fwrite(core_output, 1u, STATE_BYTES, state_file) != STATE_BYTES ||
                fclose(state_file) != 0) {
                free(expected_states); free(expected_main); free(expected_ipc); return 1;
            }
        }
#endif
        (void)snprintf(label, sizeof(label), "mixed-command thousand-step[%u] CoreDynamic first 0x108", step + 1u);
        mismatch |= compare_blob(label, core_output, expected_states + step * STATE_BYTES, STATE_BYTES);
        (void)snprintf(label, sizeof(label), "mixed-command thousand-step[%u] main output", step + 1u);
        ...
        (void)snprintf(label, sizeof(label), "mixed-command thousand-step[%u] IPC payload", step + 1u);
        mismatch |= compare_blob(label, actual_ipc.raw + DP_IPC_FLOAT_BASE,
                                 expected_ipc + step * IPC_PAYLOAD_BYTES, IPC_PAYLOAD_BYTES);
        (void)snprintf(label, sizeof(label),
                       "mixed-command thousand-step[%u] global y", step + 1u);
    ...
    free(expected_global_y);
    if (mismatch == 0) {
        puts("dyn_main mixed-command thousand-step original-ELF compare: PASS (bitwise)");
    }
