...
int main(void)
{
    ...
    if (gold_state == NULL || gold_y == NULL || gold_main == NULL || gold_ipc == NULL ||
        read_gold("gold_h34_circular_equatorial_thousand_first_state.bin", gold_state,
                  H34_STEP_COUNT * H34_STATE_BYTES) != 0 ||
        read_gold("gold_h34_circular_equatorial_thousand_first_global_y.bin", gold_y,
                  H34_STEP_COUNT * H34_GLOBAL_Y_BYTES) != 0 ||
        read_gold("gold_h34_circular_equatorial_thousand_first_out.bin", gold_main,
                  H34_STEP_COUNT * H34_MAIN_BYTES) != 0 ||
        read_gold("gold_h34_circular_equatorial_thousand_first_ipc_payload.bin", gold_ipc,
    ...
    free(gold_ipc);
    if (mismatch == 0) {
        (void)puts("PASS: H34 circular-equatorial thousand-step bitwise compare");
    }
