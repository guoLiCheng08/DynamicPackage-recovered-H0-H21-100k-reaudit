...
int main(void)
{
    ...
    if (expected_states == NULL || expected_main == NULL || expected_ipc == NULL ||
        ...
        read_gold("gold_sada_saturation_thousand_step_state.bin", expected_states, STEP_COUNT * STATE_BYTES) ||
        read_gold("gold_sada_saturation_thousand_step_out.bin", expected_main, STEP_COUNT * MAIN_BYTES) ||
        read_gold("gold_sada_saturation_thousand_step_ipc_payload.bin", expected_ipc, STEP_COUNT * IPC_PAYLOAD_BYTES) ||
        read_gold("gold_sada_saturation_h24_global_y_thousand_first_global_y.bin", expected_global_y,
    ...
    command.sada_command_angle[0] = 1.0;
    command.sada_command_angle[1] = -1.0;
    srand(1u);

    for (step = 0u; step < STEP_COUNT; ++step) {
        char label[96];
        dyn_main(&main_output, core_output, &command);
        (void)snprintf(label, sizeof(label), "SADA saturation thousand-step[%u] CoreDynamic first 0x108", step + 1u);
        mismatch |= compare_blob(label, core_output, expected_states + step * STATE_BYTES, STATE_BYTES);
        (void)snprintf(label, sizeof(label), "SADA saturation thousand-step[%u] main output", step + 1u);
        ...
        (void)snprintf(label, sizeof(label), "SADA saturation thousand-step[%u] IPC payload", step + 1u);
        mismatch |= compare_blob(label, actual_ipc.raw + DP_IPC_FLOAT_BASE,
                                 expected_ipc + step * IPC_PAYLOAD_BYTES, IPC_PAYLOAD_BYTES);
        (void)snprintf(label, sizeof(label), "H5 H24 global-y thousand-step[%u] global y", step + 1u);
    ...
    free(expected_global_y);
    if (mismatch == 0) {
        puts("dyn_main SADA-saturation thousand-step original-ELF compare: PASS (bitwise)");
    }
