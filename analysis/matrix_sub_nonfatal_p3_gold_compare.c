...
int main(void)
{
    ...
    double left_data[8] = {11.0,22.0,90.0,53.0,64.0,91.0,0.0,0.0};
    ...
    if (read_exact("gold_matrix_sub_nonfatal_p3_results.bin", expected_results,
                   sizeof(expected_results)) != 0 ||
        read_exact("gold_matrix_sub_nonfatal_p3_left.bin", expected_left,
    ...
    mismatch |= compare_blob("matrix_sub P3 returns", actual_results, expected_results,
                             sizeof(actual_results));
    mismatch |= compare_blob("matrix_sub P3 left", (const unsigned char *)left_data,
                             (const unsigned char *)expected_left, sizeof(left_data));
    if (mismatch == 0) puts("matrix_sub nonfatal P3 original-ELF compare: PASS (bitwise)");
