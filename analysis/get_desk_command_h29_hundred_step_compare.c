...
int main(void)
{
    ...
    if (expected_flags == NULL || expected_init_tail == NULL || expected_drc == NULL ||
        read_gold("gold_get_desk_command_h29_hundred_first_flags.bin", expected_flags,
                  STEP_COUNT * FLAGS_BYTES) != 0 ||
        read_gold("gold_get_desk_command_h29_hundred_first_init_tail.bin", expected_init_tail,
                  STEP_COUNT * INIT_TAIL_BYTES) != 0 ||
        read_gold("gold_get_desk_command_h29_hundred_first_drc.bin", expected_drc,
    ...
    free(expected_drc);
    if (mismatch == 0) {
        (void)puts("PASS: H29 getDeskCommand hundred-step bitwise compare");
        return 0;
    }
    return 1;
}
