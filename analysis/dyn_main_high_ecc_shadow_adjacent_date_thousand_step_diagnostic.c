...
#define STEP_COUNT 20u
#else
#define STATE_GOLD "gold_high_ecc_shadow_adjacent_date_thousand_step_state.bin"
#define MAIN_GOLD "gold_high_ecc_shadow_adjacent_date_thousand_step_out.bin"
#define IPC_GOLD "gold_high_ecc_shadow_adjacent_date_thousand_step_ipc_payload.bin"
#define SCENARIO_LABEL "high-eccentric shadow adjacent-date alternate-quaternion full-flex thousand-step"
...
static void setup_initial(DpInitialConditions *initial)
{
    ...
    initial->time_parameters[2] = 30.0;
...
int main(void)
{
    ...
    if (expected_states == NULL || expected_main == NULL || expected_ipc == NULL ||
        ...
        read_gold("gold_high_ecc_shadow_adjacent_date_thousand_step_global_y.bin", expected_global_y,
    ...
    initial_state.position_gci.x = -12000000.0;
    initial_state.position_gci.y = 65000000.0;
    initial_state.position_gci.z = 28000000.0;
    initial_state.velocity_gci.x = 0.0;
    initial_state.velocity_gci.y = 2000.0;
...
/* 原 ELF 联合探针保持零设备命令。 */
    ...
    for (step = 0u; step < STEP_COUNT; ++step) {
        ...
        if (step == 100u) {
            ...
            snapshot = fopen("/tmp/h0_step101_actual_core.bin", "wb");
            if (snapshot == NULL || fwrite(core_output, 1u, STATE_BYTES, snapshot) != STATE_BYTES || fclose(snapshot) != 0) return 2;
            snapshot = fopen("/tmp/h0_step101_expected_core.bin", "wb");
            if (snapshot == NULL || fwrite(expected_states + step * STATE_BYTES, 1u, STATE_BYTES, snapshot) != STATE_BYTES || fclose(snapshot) != 0) return 2;
            snapshot = fopen("/tmp/h0_step101_actual_main.bin", "wb");
            if (snapshot == NULL || fwrite(main_output.raw, 1u, MAIN_BYTES, snapshot) != MAIN_BYTES || fclose(snapshot) != 0) return 2;
            snapshot = fopen("/tmp/h0_step101_expected_main.bin", "wb");
            if (snapshot == NULL || fwrite(expected_main + step * MAIN_BYTES, 1u, MAIN_BYTES, snapshot) != MAIN_BYTES || fclose(snapshot) != 0) return 2;
            memset(&actual_ipc, 0, sizeof(actual_ipc));
            dp_send_dyn_tele(&actual_ipc, &main_output, 0u, 0u, 0u, 0u, 0u);
            snapshot = fopen("/tmp/h0_step101_actual_ipc.bin", "wb");
            if (snapshot == NULL || fwrite(actual_ipc.raw + DP_IPC_FLOAT_BASE, 1u, IPC_PAYLOAD_BYTES, snapshot) != IPC_PAYLOAD_BYTES || fclose(snapshot) != 0) return 2;
            snapshot = fopen("/tmp/h0_step101_expected_ipc.bin", "wb");
            if (snapshot == NULL || fwrite(expected_ipc + step * IPC_PAYLOAD_BYTES, 1u, IPC_PAYLOAD_BYTES, snapshot) != IPC_PAYLOAD_BYTES || fclose(snapshot) != 0) return 2;
            snapshot = fopen("/tmp/h0_step101_actual_y.bin", "wb");
            if (snapshot == NULL || fwrite(y, 1u, STATE_BYTES, snapshot) != STATE_BYTES || fclose(snapshot) != 0) return 2;
            snapshot = fopen("/tmp/h0_step101_expected_y.bin", "wb");
            if (snapshot == NULL || fwrite(expected_global_y + step * STATE_BYTES, 1u, STATE_BYTES, snapshot) != STATE_BYTES || fclose(snapshot) != 0) return 2;
            return 0;
        }
