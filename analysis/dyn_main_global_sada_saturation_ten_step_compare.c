...
int main(void)
{
    ...
    if (expected_states == NULL || expected_main == NULL || expected_ipc == NULL ||
        ...
        read_gold("gold_sada_saturation_ten_step_state.bin", expected_states, STEP_COUNT * STATE_BYTES) ||
        read_gold("gold_sada_saturation_ten_step_out.bin", expected_main, STEP_COUNT * MAIN_BYTES) ||
        read_gold("gold_sada_saturation_ten_step_ipc_payload.bin", expected_ipc, STEP_COUNT * IPC_PAYLOAD_BYTES)) {
    ...
    for (step = 0u; step < STEP_COUNT; ++step) {
        ...
        (void)snprintf(label, sizeof(label), "SADA saturation ten-step[%u] CoreDynamic first 0x108", step + 1u);
        mismatch |= compare_blob(label, core_output, expected_states + step * STATE_BYTES, STATE_BYTES);
        (void)snprintf(label, sizeof(label), "SADA saturation ten-step[%u] main output", step + 1u);
        ...
        (void)snprintf(label, sizeof(label), "SADA saturation ten-step[%u] IPC payload", step + 1u);
    ...
    free(expected_ipc);
    if (mismatch == 0) {
        puts("dyn_main SADA-saturation ten-step original-ELF compare: PASS (bitwise)");
    }
