...
/* 原 ELF 联合探针保持零设备命令。 */
    ...
    for (step = 0u; step < STEP_COUNT; ++step) {
        ...
        if (step == 101u) {
            FILE *dss_file = fopen("/tmp/h0_step102_actual_dss.bin", "wb");
            FILE *core_file = fopen("/tmp/h0_step102_actual_core.bin", "wb");
            FILE *main_file = fopen("/tmp/h0_step102_actual_main.bin", "wb");
            double dydt[33] = {0.0};
            double position_data[3] = {y[7], y[8], y[9]};
            double acceleration_data[3] = {0.0};
            double ecef_data[3] = {0.0};
            ...
            differential_equation(dydt, y);
            TimeArrayGet((double *)&calendar);
            GCI2ECEF(&ecef, &position, &calendar);
            orbit_dynamic(&acceleration, &position);
            dydt_file = fopen("/tmp/h0_step102_actual_precore_dydt.bin", "wb");
            orbit_file = fopen("/tmp/h0_step102_actual_orbit_acc.bin", "wb");
            ecef_file = fopen("/tmp/h0_step102_actual_ecef_position.bin", "wb");
            if (dss_file == NULL || core_file == NULL || main_file == NULL || dydt_file == NULL ||
                orbit_file == NULL || ecef_file == NULL ||
                fwrite(DeviceMeasure.dss, 1u, sizeof(DeviceMeasure.dss), dss_file) !=
                    sizeof(DeviceMeasure.dss) ||
                fwrite(core_output, 1u, sizeof(core_output), core_file) != sizeof(core_output) ||
                fwrite(main_output.raw, 1u, sizeof(main_output.raw), main_file) !=
                    sizeof(main_output.raw) ||
                fwrite(dydt, 1u, sizeof(dydt), dydt_file) != sizeof(dydt) ||
                fwrite(acceleration_data, 1u, sizeof(acceleration_data), orbit_file) !=
                    sizeof(acceleration_data) ||
                fwrite(ecef_data, 1u, sizeof(ecef_data), ecef_file) != sizeof(ecef_data) ||
                fclose(dss_file) != 0 || fclose(core_file) != 0 || fclose(main_file) != 0 ||
                fclose(dydt_file) != 0 || fclose(orbit_file) != 0 || fclose(ecef_file) != 0) {
                return 2;
            }
            break;
        }
