...
int main(void)
{
    ...
    if (gold_state == NULL || gold_y == NULL || gold_main == NULL || gold_ipc == NULL ||
        read_gold("gold_h35_near_parabolic_thousand_first_state.bin", gold_state,
                  H35_STEP_COUNT * H35_STATE_BYTES) != 0 ||
        read_gold("gold_h35_near_parabolic_thousand_first_global_y.bin", gold_y,
                  H35_STEP_COUNT * H35_GLOBAL_Y_BYTES) != 0 ||
        read_gold("gold_h35_near_parabolic_thousand_first_out.bin", gold_main,
                  H35_STEP_COUNT * H35_MAIN_BYTES) != 0 ||
        read_gold("gold_h35_near_parabolic_thousand_first_ipc_payload.bin", gold_ipc,
    ...
    free(gold_ipc);
    if (mismatch == 0) {
        (void)puts("PASS: H35 near-parabolic thousand-step bitwise compare");
    }
