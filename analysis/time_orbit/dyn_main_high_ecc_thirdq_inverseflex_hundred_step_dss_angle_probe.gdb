...
set $dp_dss_angle_counter = 0
break *UpdateSunSensorValidFlag+0x69
commands
  silent
  set $dp_dss_angle_counter = $dp_dss_angle_counter + 1
  set $dp_dss_sensor = ($dp_dss_angle_counter - 1) % 2
  if $step == 67 || $step == 72 || $step == 84
    printf "H11_ORIG_DSS_ANGLE step=%u sensor=%u angle=%a threshold=%a\\n", $step + 1, $dp_dss_sensor, $xmm0.v2_double[0], $xmm1.v2_double[0]
  end
  continue
end
break main
