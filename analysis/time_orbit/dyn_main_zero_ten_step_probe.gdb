...
commands
  ...
  while $step < 10
    ...
    if $step == 0
      dump binary memory /home/ubuntu/dynamicpackage_recovered_work/analysis/time_orbit/gold_zero_ten_step_state.bin $state ($state+33)
      dump binary memory /home/ubuntu/dynamicpackage_recovered_work/analysis/time_orbit/gold_zero_ten_step_out.bin $out ($out+0x220)
    else
      append binary memory /home/ubuntu/dynamicpackage_recovered_work/analysis/time_orbit/gold_zero_ten_step_state.bin $state ($state+33)
      append binary memory /home/ubuntu/dynamicpackage_recovered_work/analysis/time_orbit/gold_zero_ten_step_out.bin $out ($out+0x220)
    end
    call ((void (*)(void *, void *))sendDynTele)(0,$out)
    if $step == 0
      dump binary memory /home/ubuntu/dynamicpackage_recovered_work/analysis/time_orbit/gold_zero_ten_step_ipc_payload.bin $payload ($shared+0xbf0)
    else
      append binary memory /home/ubuntu/dynamicpackage_recovered_work/analysis/time_orbit/gold_zero_ten_step_ipc_payload.bin $payload ($shared+0xbf0)
    end
    set $step = $step + 1
  end
  kill
