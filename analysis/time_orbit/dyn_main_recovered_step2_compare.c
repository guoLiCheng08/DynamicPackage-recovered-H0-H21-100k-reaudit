...
int main(void)
{
    ...
    if (read_blob("gold_full_dyn_step1_post_y.bin", &state, sizeof(state)) != 0 ||
        read_blob("gold_full_dyn_step1_sts.bin", devices.sts, sizeof(devices.sts)) != 0 ||
        read_blob("gold_full_dyn_step1_gyro.bin", devices.gyro, sizeof(devices.gyro)) != 0 ||
        read_blob("gold_full_dyn_step1_dss.bin", devices.dss, sizeof(devices.dss)) != 0 ||
        read_blob("gold_full_dyn_step1_magmeter.bin", devices.magmeter,
                  sizeof(devices.magmeter)) != 0 ||
        read_blob("gold_full_dyn_step1_gps.bin", &devices.gps, sizeof(devices.gps)) != 0 ||
        read_blob("gold_full_dyn_step1_wheels.bin", wheels, sizeof(wheels)) != 0 ||
        read_blob("gold_full_dyn_step1_sada.bin", &sada, sizeof(sada)) != 0 ||
        read_blob("gold_full_dyn_init_mtq.bin", mtq, sizeof(mtq)) != 0 ||
        read_blob("gold_dyn_main_step2_out.bin", &expected_out, sizeof(expected_out)) != 0 ||
        read_blob("gold_dyn_main_step2_ipc.bin", &expected_ipc, sizeof(expected_ipc)) != 0 ||
...
#endif
    if (compare_bytes("dyn_main frame", &actual_out, &expected_out, sizeof(actual_out)) != 0 ||
    ...
    puts("recovered dyn_main second-step original-ELF compare: PASS (frame/IPC bitwise)");
    return 0;
}
