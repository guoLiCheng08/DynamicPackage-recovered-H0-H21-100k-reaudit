...
commands
  ...
  # 仅在运行期内修补调试副本的未使用 vector_print 代码槽：
  # vector2angle 入口记录 XMM2 至 angle_slot，复制被跳转覆盖的 5 B，
  # 随后跳回 vector2angle+5。原 input/DynamicPackage.elf 不会被写入。
  set $angle_slot=(double *)calloc(1,8)
  set $v2a=(char *)vector2angle
  set $cave=(char *)vector_print
  set {unsigned char}($cave+0)=0xf2
  set {unsigned char}($cave+1)=0x0f
  set {unsigned char}($cave+2)=0x11
  set {unsigned char}($cave+3)=0x15
  set {int}($cave+4)=(int)((char *)$angle_slot-($cave+8))
  set {unsigned char}($cave+8)=0x55
  set {unsigned char}($cave+9)=0x53
  set {unsigned char}($cave+10)=0x48
  set {unsigned char}($cave+11)=0x89
  set {unsigned char}($cave+12)=0xf3
  set {unsigned char}($cave+13)=0x48
  set {unsigned char}($cave+14)=0x89
  set {unsigned char}($cave+15)=0xfd
  set {unsigned char}($cave+16)=0x48
  set {unsigned char}($cave+17)=0x83
  set {unsigned char}($cave+18)=0xec
  set {unsigned char}($cave+19)=0x18
  set {unsigned char}($cave+20)=0xe9
  set {int}($cave+21)=(int)(($v2a+12)-($cave+25))
  set {unsigned char}($v2a+0)=0xe9
  set {int}($v2a+1)=(int)($cave-($v2a+5))
  set $step=0
  while $step<100
    call ((void (*)(void *,void *,void *))dyn_main)($out,$state,$cmd)
    if $step==0
      dump binary memory /tmp/h11_original_second_dss_angle_trace.bin $angle_slot ($angle_slot+1)
    else
      append binary memory /tmp/h11_original_second_dss_angle_trace.bin $angle_slot ($angle_slot+1)
    end
    if $step==0
