...
static void advance_state(double out[DP_STATE_DIM], const double base[DP_STATE_DIM],
                          const double derivative[DP_STATE_DIM], double scale)
{
    unsigned index;
    for (index = 0u; index < DP_STATE_DIM; ++index) out[index] = base[index] + scale * derivative[index];
}
...
int main(void)
{
    ...
    if (read_blob("gold_sada_position_step1_post_y.bin", &initial, sizeof(initial)) != 0 ||
    ...
    file = fopen("/tmp/e10_recovered_sada_step2_flex_stages.bin", "wb");
    if (file == NULL) return 1;
    for (stage = 0u; stage < 4u; ++stage) {
        if (fwrite(&derivative[stage][4], sizeof(double), 3u, file) != 3u ||
            ...
            fwrite(&derivative[stage][23], sizeof(double), 10u, file) != 10u) {
            (void)fclose(file);
            return 1;
        }
    }
