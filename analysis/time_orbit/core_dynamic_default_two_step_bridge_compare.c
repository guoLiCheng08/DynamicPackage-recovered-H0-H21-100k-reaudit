                         ...
                         const char *expected_file)
{
    ...
    unsigned index;
    if (read_blob(expected_file, expected, sizeof(expected)) != 0) return 1;
    for (index = 0u; index < DP_STATE_DIM; ++index) {
        ...
        if (a != e) {
            fprintf(stderr, "%s state[%u] got=%016llx expected=%016llx\n", label, index,
...
int main(void)
{
    ...
    if (dp_core_default_rk4_step(&model, state, &time, 0.1) != 0 ||
        compare_state("step1", state, "gold_core_dynamic_y_step1.bin") != 0 ||
        dp_core_default_rk4_step_with_terms(&model, state, &time, 0.1,
                                             &wheel_angular_momentum, &total_torque) != 0 ||
        compare_state("step2", state, "gold_core_dynamic_y_step2.bin") != 0) return 1;
    puts("CoreDynamic default two-step bridge original-ELF compare: PASS (bitwise)");
    return 0;
}
