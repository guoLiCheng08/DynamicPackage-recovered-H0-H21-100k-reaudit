...
commands
  ...
  # 仅在调试副本内的未使用 vector_print 槽写入返回值记录跳板。
  # isEarthShadow+0x51 已执行 seta al；跳板保存 eax 并完成原函数尾声。
  set $shadow_slot=(int *)calloc(1,4)
  set $shadow=(char *)isEarthShadow
  set $cave=(char *)vector_print
  set {unsigned char}($cave+0)=0x89
  set {unsigned char}($cave+1)=0x05
  set {int}($cave+2)=(int)((char *)$shadow_slot-($cave+6))
  set {unsigned char}($cave+6)=0x48
  set {unsigned char}($cave+7)=0x83
  set {unsigned char}($cave+8)=0xc4
  set {unsigned char}($cave+9)=0x18
  set {unsigned char}($cave+10)=0x5b
  set {unsigned char}($cave+11)=0x5d
  set {unsigned char}($cave+12)=0xc3
  set {unsigned char}($shadow+0x51)=0xe9
  set {int}($shadow+0x52)=(int)($cave-($shadow+0x56))
  set $step=0
  while $step<100
    call ((void (*)(void *,void *,void *))dyn_main)($out,$state,$cmd)
    if $step==0
      dump binary memory /tmp/h11_original_earth_shadow_trace.bin $shadow_slot ((char *)$shadow_slot+4)
    else
      append binary memory /tmp/h11_original_earth_shadow_trace.bin $shadow_slot ((char *)$shadow_slot+4)
    end
    if $step==0
