...
int main(void)
{
    ...
    /* WheelGroup+0x68 的原 ELF 3×4 行主序安装矩阵位模式常量。 */
    double wheel_map_data[12] = {
        0x1.3986a1562d39ep-1, 0x1.3986a1562d39ep-1,
        -0x1.3986a1562d3a7p-1, -0x1.3986a1562d3a7p-1,
        0x1.3986a1562d39ep-1, -0x1.3986a1562d3a7p-1,
        -0x1.3986a1562d3a7p-1, 0x1.3986a1562d39ep-1,
        -0x1.0000000000000p-1, -0x1.0000000000000p-1,
        -0x1.0000000000000p-1, -0x1.0000000000000p-1
    };
    double step1_core_input[18];
    ...
    if (read_blob("gold_rw1_pos_step1_post_y.bin", &state, sizeof(state)) != 0 ||
        read_blob("gold_rw1_pos_step1_sts.bin", devices.sts, sizeof(devices.sts)) != 0 ||
        read_blob("gold_rw1_pos_step1_gyro.bin", devices.gyro, sizeof(devices.gyro)) != 0 ||
        read_blob("gold_rw1_pos_step1_dss.bin", devices.dss, sizeof(devices.dss)) != 0 ||
        read_blob("gold_rw1_pos_step1_magmeter.bin", devices.magmeter,
                  sizeof(devices.magmeter)) != 0 ||
        read_blob("gold_rw1_pos_step1_gps.bin", &devices.gps, sizeof(devices.gps)) != 0 ||
        read_blob("gold_rw1_pos_step1_wheels.bin", wheels, sizeof(wheels)) != 0 ||
        read_blob("gold_rw1_pos_step1_sada.bin", &sada, sizeof(sada)) != 0 ||
        read_blob("gold_full_dyn_init_mtq.bin", mtq, sizeof(mtq)) != 0 ||
        read_blob("gold_rw1_pos_step2_post_y.bin", &expected_state, sizeof(expected_state)) != 0 ||
        read_blob("gold_rw1_pos_step2_out.bin", &expected_out, sizeof(expected_out)) != 0 ||
        read_blob("gold_rw1_pos_step2_ipc.bin", &expected_ipc, sizeof(expected_ipc)) != 0 ||
        read_blob("gold_rw1_pos_step1_core_input.bin", step1_core_input,
                  sizeof(step1_core_input)) != 0 ||
    ...
    /* 第一轮 rw1_pos 的 SetWheelAcc 在传感器前消耗一次基础高斯噪声。 */
    { const double warmup_rw_command[DP_WHEEL_COUNT] = {0.001, 0.0, 0.0, 0.0};
      dp_set_wheel_acc(warmup_wheels, warmup_rw_command, NULL, NULL); }
    ...
    e8_rand_stage = 1;
#endif
    memcpy(wheel_h_data, &step1_core_input[3], sizeof(wheel_h_data));
    ...
    {
        FILE *debug_file = fopen("/tmp/e9_rw1_actual_out.bin", "wb");
        if (debug_file == NULL || fwrite(&actual_out, 1u, sizeof(actual_out), debug_file) !=
            sizeof(actual_out) || fclose(debug_file) != 0) return 1;
    }
    {
        FILE *debug_file = fopen("/tmp/e9_rw1_actual_wheel_terms.bin", "wb");
        if (debug_file == NULL || fwrite(wheel_h_data, sizeof(double), 3u, debug_file) != 3u ||
            fwrite(wheel_torque_data, sizeof(double), 3u, debug_file) != 3u || fclose(debug_file) != 0) return 1;
    }
#ifdef E8_COUNT_RAND
    (void)fprintf(stderr, "recovered UpdateDeviceMeasure gaussian2 calls=%u\\n", e8_gaussian2_calls);
#endif
    if (compare_bytes("rw1_pos state", &state, &expected_state, sizeof(state)) != 0 ||
        compare_bytes("rw1_pos frame", &actual_out, &expected_out, sizeof(actual_out)) != 0 ||
    ...
    puts("recovered dyn_main rw1_pos second-step original-ELF compare: PASS (state/frame/IPC bitwise)");
    return 0;
}
