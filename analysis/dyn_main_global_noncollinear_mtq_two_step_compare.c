...
int main(void)
{
    ...
    if (read_blob("gold_sensor_init_gyro.bin", DeviceMeasure.gyro,
        ...
        read_blob("gold_noncollinear_mtq_step1_state.bin", expected_core_prefix,
                  sizeof(expected_core_prefix)) != 0 ||
        read_blob("gold_noncollinear_mtq_step1_out.bin", &expected_main,
                  sizeof(expected_main)) != 0 ||
        read_blob("gold_noncollinear_mtq_step2_state.bin", expected_core_prefix_step2,
                  sizeof(expected_core_prefix_step2)) != 0 ||
        read_blob("gold_noncollinear_mtq_step2_out.bin", &expected_main_step2,
                  sizeof(expected_main_step2)) != 0 ||
        read_blob("gold_noncollinear_mtq_step1_ipc.bin", &expected_ipc_step1,
                  sizeof(expected_ipc_step1)) != 0 ||
        read_blob("gold_noncollinear_mtq_step2_ipc.bin", &expected_ipc_step2,
    ...
    mismatch |= compare_blob("dyn_main step2 IPC payload", actual_ipc.raw + DP_IPC_FLOAT_BASE,
                             ...
                             DP_IPC_SHM_BYTES - DP_IPC_FLOAT_BASE);
    if (mismatch == 0) {
        puts("dyn_main noncollinear-MTQ two-step original-ELF compare: PASS (bitwise)");
    }
